    ...
    <div className="items-center justify-between w-full flex flex-col">
      ...
      <div className="items-start self-center justify-start lg:space-x-2 cursor-pointer w-full space-y-4  lg:space-y-0 lg:flex py-2 px-3">
        ...
        {Array.from({length:2},(_,i)=>(
          <div onClick={()=> setActiveTab(i)} className={`self-center py-2 px-8 rounded-md text-lg ${activeTab === i ? "bg-black text-gray-100" : "text-black bg-gray-100"}`}>
            {i === 0 ? "statments" : "Statistics"}
          </div>
        ))}
        <div className="items-center justify-end flex space-x-6 py-3 px-4 w-full h-full">
      ...
      {/* items */}
      {activeTab === 0 ? (<div>No statements</div>):( <div className="h-[300px] overflow-y-scroll scrollbar-hide w-full py-3 px-4">
        {Data.map((item)=>(
          <div className="flex w-full items-center justify-between py-6 py-3 px-3">
            ...
            {/* progress */}
         <Progress className="w-[25%] lg:inline hidden" colorScheme='crimson' size='sm' value={item.value} />   
